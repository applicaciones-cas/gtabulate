...
public class Scoring extends Transaction {
    ...
    public ContestMaster ContestMaster() {
    ...
    public Model_Event_Tabulation_Detail getDetail(int criteriaNo) {
        if (getMaster().getGroupId().isEmpty()
                || getMaster().getComputerId().isEmpty()) {
            return null;
        }

        if (criteriaNo <= 0 || criteriaNo > paCriteria.size()) {
            return null;
        }
        ...
        for (int lnCtr = 0; lnCtr <= paDetail.size() - 1; lnCtr++) {
            ...
            if (loDetail.getEntryNo() == criteriaNo) {
                return loDetail;
            }
        }
    ...
    public JSONObject initTransaction() throws GuanzonException, SQLException {
        ...
        poContestMaster = new TabulationControllers(poGRider, logwrapr).ContestMaster();
        poContestMaster.setRecordStatus(RecordStatus.ACTIVE);
        poContestMaster.initialize();
        poContestMaster.setWithParentClass(true);
